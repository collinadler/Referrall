...
@implementation CPAOnboardingViewController {
    NSURL *_videoURL;
    UIPageViewController *_pageVC;
    
    CPAOnboardingContentViewController *_currentPage;
    CPAOnboardingContentViewController *_upcomingPage;
}
...
+ (instancetype)onboardWithBackgroundImage:(UIImage *)backgroundImage contents:(NSArray *)contents {
    CPAOnboardingViewController *onboardingVC = [[self alloc] initWithBackgroundImage:backgroundImage contents:contents];
    return onboardingVC;
}
...
+ (instancetype)onboardWithBackgroundVideoURL:(NSURL *)backgroundVideoURL contents:(NSArray *)contents {
    CPAOnboardingViewController *onboardingVC = [[self alloc] initWithBackgroundVideoURL:backgroundVideoURL contents:contents];
    return onboardingVC;
}
...
- (instancetype)initWithContents:(NSArray *)contents {
    ...
    [self.joinButton setAttributedTitle:[[NSAttributedString alloc] initWithString:@"Join now" attributes:@{NSFontAttributeName : [UIFont fontWithName:@"HelveticaNeue-Medium" size:13.0f],
                                                                                                                    NSForegroundColorAttributeName : [CPAConstants skyBlueColor ]}]
                                        forState:UIControlStateNormal];
    [self.joinButton addTarget:self action:@selector(buttonPressed:) forControlEvents:UIControlEventTouchUpInside];

    self.signInButton = [UIButton buttonWithType:UIButtonTypeCustom];
    self.signInButton.backgroundColor = [CPAConstants skyBlueColor];
    self.signInButton.layer.cornerRadius = 2;
    [self.signInButton setAttributedTitle:[[NSAttributedString alloc] initWithString:@"Sign in" attributes:@{NSFontAttributeName : [UIFont fontWithName:@"HelveticaNeue-Medium" size:13.0f],
                                                                                                            NSForegroundColorAttributeName : [UIColor whiteColor]}]
                               forState:UIControlStateNormal];
    [self.signInButton addTarget:self action:@selector(buttonPressed:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)generateView {
    ...
    // and auto-navigation
    for (CPAOnboardingContentViewController *contentVC in self.viewControllers) {
    ...
    // Congifure the bottome bottoms
    CGFloat padding = 5;
    CGFloat buttonWidth = (CGRectGetWidth(self.view.bounds) - (padding * 3)) / 2;
    CGFloat buttonHeight = 35;
    
    self.joinButton.frame = CGRectMake(padding, CGRectGetMaxY(self.view.frame) - buttonHeight - padding, buttonWidth, buttonHeight);
    [self.view addSubview:self.joinButton];
    self.signInButton.frame = CGRectMake(CGRectGetMaxX(self.joinButton.frame) + padding, CGRectGetMaxY(self.view.frame) - buttonHeight - padding, buttonWidth, buttonHeight);
    [self.view addSubview:self.signInButton];
    
    // create and configure the page control
    if (!self.hidePageControl) {
        self.pageControl.frame = CGRectMake(0, CGRectGetMinY(self.joinButton.frame) - self.underPageControlPadding - kPageControlHeight, self.view.frame.size.width, kPageControlHeight);
...
- (void)buttonPressed:(UIButton *)button {
    CPALoginViewController *loginVC = [[CPALoginViewController alloc] init];
    if (button == self.joinButton) {
        loginVC.state = CPALoginViewControllerStateSignUp;
    } else if (button == self.signInButton) {
        loginVC.state = CPALoginViewControllerStateLogin;
    }
